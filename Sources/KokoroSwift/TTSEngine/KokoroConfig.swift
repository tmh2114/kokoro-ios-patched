...
struct KokoroConfig: Decodable {
  ...
  nonisolated static func loadConfig() throws -> KokoroConfig {
    guard let fileURL = Bundle.module.url(forResource: "config", withExtension: "json", subdirectory: "Resources") else {
      throw NSError(domain: "KokoroConfig", code: -1, userInfo: [NSLocalizedDescriptionKey: "config.json not found in bundle"])
    }

    let configJSON = try String(contentsOf: fileURL, encoding: .utf8)

    guard let data = configJSON.data(using: .utf8) else {
      throw NSError(domain: "KokoroConfig", code: -2, userInfo: [NSLocalizedDescriptionKey: "Failed to encode config.json as UTF-8"])
    }
